...
cadena_t mayores(cadena_t cad, unsigned int k, unsigned int max){
  ...
  if(!es_vacia_cadena(cad)){
    int maxi = max;
    cola_prioridad_t qe = crear_cp(k, maxi);
    localizador_t loc = inicio_cadena(cad);
    while(es_localizador(loc)){
      int num = numero_info(info_cadena(loc,cad));
      if(num > 0 && num <= maxi && !hay_prioridad(num, qe)){
        int prioritario = prioridad_prioritario(qe);
        if(prioritario < num && es_llena_cp(qe))
          eliminar_prioritario(qe);
        if(!es_llena_cp(qe))
          insertar_en_cp(copia_info(info_cadena(loc,cad)), num, qe);
      }
      loc = siguiente(loc, cad);
    }
    while(!es_vacia_cp(qe)){
      insertar_al_final(copia_info(prioritario(qe)), chain);
      eliminar_prioritario(qe);
    }
    liberar_cp(qe);
  }
