...
void remove_segment(localizador_t &from, localizador_t &until, cadena_t &chain){
  while(precede_en_cadena(from, until, chain)){
    localizador_t aux = from->siguiente;
    remover_de_cadena(from, chain);
    from = aux;
  }
}

cadena_t separar_segmento(localizador_t desde, localizador_t hasta, cadena_t &cad){
  cadena_t chain = segmento_cadena(desde, hasta, cad);
  remove_segment(desde, hasta, cad); 
  return chain;
}
